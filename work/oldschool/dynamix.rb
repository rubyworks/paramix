...
class Module
  ...
  def dynamic_features ; @dynamic_features ||= [] ; end

  # Create a dynamic feature.
  def dynamic_feature( &blk )
    adhoc = (class << self; self; end)
    adhoc.class_eval {
...
=begin
  ...
  def append_dynamic_features( base, options )
    ...
    # dynamic features
    dynamic_features.each do |mf|
      adhoc = (class << base; self; end)
      adhoc.class_eval { define_method( :__EVAL__, &mf ) }
      result = base.__EVAL__(options)
      adhoc.class_eval { remove_method( :__EVAL__ ) }
      # if string block
      base.class_eval result if result.is_a?(String)
    end
  end
